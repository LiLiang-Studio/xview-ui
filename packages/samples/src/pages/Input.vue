<template>
  <div style="width:420px;padding-bottom:200px">
    ...
    <p>基本用法，可以使用 v-model 实现数据的双向绑定。可以直接设置 style 来改变输入框的宽度，默认 100%。</p><br>
    <Input v-model="value" placeholder="Enter something..." style="width: 300px" />

    <div class="page-sub-title">尺寸</div>
    <p>输入框有三种尺寸：大、默认（中）、小;通过设置size为large和small设置为大和小尺寸，不设置为默认（中）尺寸。</p><br>
    ...
    <p>开启属性 clearable 可显示清空按钮。</p><br>
    <Input v-model="value14" placeholder="Enter something..." clearable style="width: 200px" />

    <div class="page-sub-title">输入长度限制</div>
    <p>开启属性 show-word-limit 可以显示字数统计，需配合 maxlength 属性来限制输入长度。</p><br>
    <div>
      <Input v-model="value16" maxlength="10" show-word-limit placeholder="Enter something..." style="width: 200px" />
      <Input v-model="value17" maxlength="100" show-word-limit type="textarea" placeholder="Enter something..." style="width: 200px" />
    </div>

    <div class="page-sub-title">密码框</div>
    <p>在 type="password" 时，开启属性 password 可以切换显示隐藏密码</p><br>
    <div>
    ...
    <p>通过 icon 属性可以在输入框右边加一个图标。点击图标，会触发 on-click 事件。</p><br>
    <Input
      v-model="value4"
    ...
    <p>除了设置 icon，也可以通过设置 prefix 和 suffix 及同名 slot 设置前缀及后缀图标。</p><br>
    <div>
      Props：
    ...
    <p>开启 search 属性，可以设置为搜索型输入框。</p><br>
    ...
    <p>通过设置属性 type 为 textarea 来使用文本域，用于多行输入。通过设置属性 rows 控制文本域默认显示的行数。</p><br>
    ...
    <p>设置属性 autosize，文本域会自动适应高度的变化。autosize也可以设定为一个对象，指定最小行数和最大行数。</p><br>
    ...
    <p>通过添加disabled属性可设置为不可用状态。</p><br>
    ...
    <p>通过前置和后置的 slot 可以实现复合型的输入框。</p><br>
    ...
    <p>结合 Poptip 组件，实现一个数值输入框，方便内容超长时的全量展现。</p><br>
